...
namespace Server_Hue
{
    ...
    class PTPSession
    {
        ...
        public void PTP_Recv(GameSession session,double ptpfirst, double ptpsecond)
        {
            bool success = true;
            ushort size = 0;
            ushort sendbyte = 0;
            ...
            packet.packetType = (ushort)packTypes.ptp;
            size += 2;
            packet.time00 = ptpfirst;
            size += 8;
            packet.time01 = ptpsecond;
            size += 8;
            packet.time02 = DateTimeOffset.Now.ToUnixTimeMilliseconds();
            size += 8;
            packet.time03 = 0;
            size += 8;
            packet.size = size;
        ...
        public void LoginPacket(GameSession session)
        {
            bool success = true;
            ushort size = 0;
            ushort sendbyte = 0;
            Login_Packet packet = new Login_Packet() { packetId = session.SessionID, packetType = (ushort)packTypes.connectPacket };
            ArraySegment<byte> s = SendBufferHelper.Open(4096);
